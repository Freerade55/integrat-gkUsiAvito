...
function addTask(int $leadId, int $respUser)
{
    ...
    $queryData = array(
        [
            "text" => " Заявка с Авито. Проверь примечание сделки и созвонись с клиентом!",
            "entity_id" => $leadId,
            "complete_till" => $res,
            "entity_type" => "leads",
...
function addNote(string $common, int $entity_id, string $clientComment, string $specialistComment, string $phone, string $fullName) {
    ...
    $queryData = array(
            ...
            [
                ...
                "note_type" => "common",
                "params" => [
                    "text" => "Имя клиента - $fullName \n \n
                    Телефон - $phone \n \n
                    Переписка с клиентом - $clientComment \n \n
                    Комментарий от специалиста - $specialistComment \n \n " 
                ]
